...
public class ConfigHandler extends Queue {
    ...
    public static void checkPlayers(Connection connection) {
        ...
        for (ServerPlayer player : NeoPlatform.onlinePlayers()) {
            final String userName = NeoPlatform.playerName(player);
            if (ConfigHandler.playerIdCache.get(userName.toLowerCase(Locale.ROOT)) == null) {
                UserStatement.loadId(connection, userName, NeoPlatform.playerUuid(player));
            }
        }
    }
    ...
    public static void loadWorlds(Statement statement) {
        try {
            ...
            for (String worldname : NeoPlatform.worldNames()) {
